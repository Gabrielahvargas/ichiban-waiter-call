ALTER TABLE public.dining_tables
  ADD COLUMN IF NOT EXISTS call_button integer NOT NULL DEFAULT 3,
  ADD COLUMN IF NOT EXISTS attend_button integer NOT NULL DEFAULT 4;

UPDATE public.dining_tables SET call_button = 3 WHERE call_button IS NULL;
UPDATE public.dining_tables SET attend_button = 4 WHERE attend_button IS NULL;

ALTER TABLE public.dining_tables
  ADD CONSTRAINT dining_tables_call_button_range CHECK (call_button BETWEEN 1 AND 4),
  ADD CONSTRAINT dining_tables_attend_button_range CHECK (attend_button BETWEEN 1 AND 4),
  ADD CONSTRAINT dining_tables_buttons_distinct CHECK (call_button <> attend_button);
...
BEGIN
  ...
  -- Resolve the physical switch number into an action using this table's mapping.
  IF COALESCE(p_click_type, 'single') <> 'single' THEN
    v_action := NULL;
  ELSIF v_table.table_number IS NULL THEN
    v_action := NULL;
  ELSIF p_button = v_table.call_button THEN
    v_action := 'call';
  ELSIF p_button = v_table.attend_button THEN
    v_action := 'attend';
  ELSE
    v_action := NULL;
  END IF;
...
DROP FUNCTION IF EXISTS public.ingest_button_event(integer, integer, app_env, text, text);
